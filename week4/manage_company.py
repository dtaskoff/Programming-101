...
def add_employee(cursor, employee):
    ...
    cursor.execute(insert_query, (id_, name, monthly_salary, yearly_bonus, position))
...
def delete_employee(cursor, employee_id):
    ...
    return "employee with id {0} was removed from the database".format(employee_id)
...
def update_employee(cursor, employee_id):
    ...
    update_query = '''UPDATE employees
                ...
                WHERE id is ?'''
    cursor.execute(update_query, (name, monthly_salary, yearly_bonus, position, employee_id))
...
def yearly_spending(cursor):
    salaries = cursor.execute('''SELECT monthly_salary,
